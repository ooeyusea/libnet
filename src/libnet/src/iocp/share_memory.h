...
namespace libnet {
	...
	public:
		...
		char* Write(uint32_t& size) {
			...
			uint32_t realIn = ((ShareMemoryHeader*)_buff)->in & (_size - 1);
			uint32_t realOut = ((ShareMemoryHeader*)_buff)->out & (_size - 1);
			if (realIn >= realOut)
		...
		inline bool WriteBlock(const void* content, const uint32_t size) {
			...
			if (size <= ((ShareMemoryHeader*)_buff)->size - realIn)
				...
				memcpy(_buff + sizeof(ShareMemoryHeader), (const char*)content + ((ShareMemoryHeader*)_buff)->size - realIn, size - (_size - realIn));
			}
			((ShareMemoryHeader*)_buff)->in += size;
			return true;
		}
